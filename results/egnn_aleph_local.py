...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0, act_fn=nn.SiLU(), residual=True, attention=False, normalize=False, recurrent=False, coords_weight=1., coords_agg='mean', tanh=False, universal_node_num=25, delta=1., exp=False, device='cpu', local=True):
    ...
    def edge_model(self, source, target, radial, embedding,edge_attr):
        ###radial === gram_dist
        if edge_attr is None and not self.local:  # Unused.
        ...
        elif edge_attr is None and self.local:
            out = torch.cat([source.to(self.device), target.to(self.device), radial.to(self.device)], dim=1).to(self.device)
        elif self.local:
            out = torch.cat([source.to(self.device), target.to(self.device), radial.to(self.device), edge_attr.to(self.device)], dim=1).to(self.device)
        else:
    ...
    def coord2gram(self, edge_index, coord):
        ...
        gram, projections = torch.split(ug_vec, 3, dim=2) #shape b x d  x (n+1)
        #apply exponential
        if self.exp:
...
class E_GCL_mask(E_GCL):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_attr_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False, delta=1., exp=False, device='cpu', local=True, coord_update=False):
        E_GCL.__init__(self, input_nf, output_nf, hidden_nf, edges_in_d=edges_in_d, nodes_att_dim=nodes_attr_dim, act_fn=act_fn, recurrent=recurrent, coords_weight=coords_weight, attention=attention, delta=delta, exp=exp, device=device, local=True)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, coords_weight=1.0, attention=False, node_attr=1, exp=False, delta=1.,h0=None, edges=None, edge_attr=None, node_mask=None, edge_mask=None, n_nodes=None, local=True, coord_update=False):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, E_GCL_mask(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf, nodes_attr_dim=n_node_attr, act_fn=act_fn, recurrent=True, coords_weight=coords_weight, attention=attention, exp=self.exp, delta=delta, local=self.local,coord_update=coord_update,device=device))
        torch.manual_seed(42)
    ...
    def forward(self, x):
        ...
        for i in range(0, self.n_layers):
            if self.node_attr:
                h, x, _ = self._modules["gcl_%d" % i](h, self.edges, x, self.node_mask, self.edge_mask, edge_attr=self.edge_attr, node_attr=self.h0, n_nodes=self.n_nodes)
            else:
                h, x, _ = self._modules["gcl_%d" % i](h, self.edges, x, self.node_mask, self.edge_mask, edge_attr=self.edge_attr,
...
def unsorted_segment_mean(data, segment_ids, num_segments):
    ...
    result.scatter_add_(0, segment_ids, data).to(device)
    count.scatter_add_(0, segment_ids, torch.ones_like(data, device=device)).to(device)
...
def main(cloud1, cloud2, dimension,n):
    ...
    torch.manual_seed(42)
    model = nn.Sequential(
        EGNN(in_node_nf=n_feat, hidden_nf=hidden_nf,  in_edge_nf=1, n_layers=20, device=device,exp=True,h0=h, edges=edges, edge_attr=edge_attr, node_mask=None, edge_mask=None, n_nodes=n_nodes, local=local, coord_update=coord_update),
    ).to(device)
    lr = 2e-4
    wd = 1e-6
    #iterations = 1000
...
if __name__ == "__main__":
    res2 = {'EGNNalephloc_coord_exp' :   {'2_body_ex_1' : None, '2_body_ex_2' : None,'2_body_ex_3' : None , '2022' : None }}
