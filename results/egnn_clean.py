...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=4, attention=False, node_attr=0):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, E_GCL(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_d=in_edge_nf, act_fn=act_fn, attention=attention))
...
if __name__ == "__main__":
    ...
    # Initialize EGNN
    egnn = EGNN(in_node_nf=n_feat, hidden_nf=32, in_edge_nf=1, n_nodes=n_nodes)
...
class E_GCL_mask(E_GCL):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_attr_dim=0, act_fn=nn.ReLU(), recurrent=True, coords_weight=1.0, attention=False, delta=1., exp=False, device='cpu'):
...
def main(cloud1, cloud2, dimension,n):
    ...
    model = nn.Sequential(
        EGNN(in_node_nf=n_feat, hidden_nf=hidden_nf,  in_edge_nf=1, n_layers=20, device=device,exp=False,h0=h, edges=edges, edge_attr=edge_attr, node_mask=None, edge_mask=None, n_nodes=n_nodes),
    ).to(device)
    lr = 2e-5
    wd = 1e-6
    #iterations = 1000
...
if __name__ == "__main__":
    res2 = {'EGNNaleph' :   {'2_body_ex_1' : None, '2_body_ex_2' : None,'2_body_ex_3' : None}}
    points = {'2_body_ex_1' : (A, B, 3,5), '2_body_ex_2' : (C, D, 3, 7),'2_body_ex_3' : (E, F, 3, 6)}
